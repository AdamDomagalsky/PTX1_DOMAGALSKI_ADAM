...
while flag do
    ...
    if(selected!='back')
        ...
        if prompt.yes?("Would you like to see 
            articles of #{selected} category?")
            selected=prompt.select("Articles qof #{selected} category", getArticles(selected),'back', per_page:14)
            if(selected=='back')
                stack.pop
                title.pop
                if(stack.empty?)
